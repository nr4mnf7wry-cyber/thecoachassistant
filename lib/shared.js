...
export const EVAL_CATEGORY_KEYS = { Physique: "cat_Physique", Technique: "cat_Technique", Defense: "cat_Defense", Mental: "cat_Mental" };
...
export const ALL_EVAL_SKILLS = Object.values(EVAL_CATEGORIES).flat();
...
export const emptyMatchStat = () => ({
  buts: 0, passes: 0, jaune: 0, rouge: 0, minutes: 0, note: "", poste: "",
...
export function useTeamData() {
  ...
  }, [players, staff, matches, trainings, evaluations, availabilities, exerciseLibrary, bodyMetrics, leagueMatches, opponentProfiles, injuries, developmentGoals, liveSessions, teamName, seasons, currentSeason, loaded]);
...
export function aggregateMatches(players, matches) {
...
export function computeMinutes(match) {
  ...
  const outMap = {}, inMap = {};
  subs.forEach((s) => { outMap[s.outId] = Number(s.minute) || 0; inMap[s.inId] = Number(s.minute) || 0; });
  const minutes = {};
  starters.forEach((id) => { minutes[id] = outMap[id] !== undefined ? outMap[id] : duration; });
  Object.keys(inMap).forEach((id) => {
    const start = inMap[id];
    const end = outMap[id] !== undefined ? outMap[id] : duration;
    minutes[id] = Math.max(0, end - start);
  });
  return minutes;
}
...
export function isPlayerUnavailable(availabilities, playerId, dateStr) {
  ...
  return (availabilities || []).find((a) => a.playerId === playerId && dateStr >= a.startDate && dateStr <= (a.endDate || a.startDate)) || null;
}
